 ...
 */
constexpr char vstbridge_window_class_name[] = "vstbridge plugin";
 ...
 * area of vstbridge windows. (so it will consider other Wine windows to the
 * right or to the bottom of a vstbridge plugin editor, but not the extended
 * client area itself)
 *
 ...
 *
 * As of vstbridge 3.0 XEmbed is back as an option, but it's disabled by default
 * because of the issues mentioned above.
 *
 * In vstbridge (upstream 3.x).0 we added another layer to the embedding structure. This is
...
class Editor {
   public:
     ...
     * window (using the plugin API). Before vstbridge (upstream 3.x).0 this was implicit.
     */
    ...
    /**
     * Whether to use XEmbed instead of vstbridge's normal window embedded. Wine
   ...
   private:
     ...
     * vstbridge (upstream 3.x), but doing it ourselves here makes things m much more
