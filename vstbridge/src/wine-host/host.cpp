...
static const std::string host_name = "vstbridge host version " +
                                     std::string(vstbridge_git_version)
#ifdef __i386__
 ...
 * plugin plugin, and then connect back to the `libvstbridge-{clap,vst2,vst3}.so`
 * instance that spawned this over the socket.
 *
 * For the group host case this process will act as a daemon vstbridge plugin
...
int VSTBRIDGE_EXPORT
    ...
    // arguments for vstbridge-host.exe. Group host processes receive only a unix
    ...
    if (is_group_host) {
        ...
        try {
            ...
            // starts multiple vstbridge instances that all use the same plugin
...
#else
                    std::cerr
                        << "This version of vstbridge has not been compiled "
                           "with CLAP support"
                        << std::endl;
                    return 1;
#endif
        ...
        Win32Thread worker_thread([&]() {
            ...
            //        this is a bug in vstbridge, Wine, or those plugins, but
