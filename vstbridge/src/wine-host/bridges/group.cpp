...
void GroupBridge::accept_requests() {
    group_socket_acceptor_.async_accept(
        [&](const std::error_code& error,
            ...
            // `vstbridge-host.exe`. We will reply with this process's PID so the
            // vstbridge plugin will be able to tell if the plugin has caused
...
std::string create_logger_prefix(const fs::path& socket_path) {
    ...
    // '/tmp/vstbridge-group-<group_name>-<wine_prefix_id>-<architecture>.sock',
    ...
    std::regex group_regexp("^vstbridge-group-(.*)-[^-]+-[^-]+$",
