 ...
 * vstbridge will try to connect to the group socket on initialization and it
 * will launch a new group host process if it can't. If this is done for
 * multiple vstbridge instances at the same time, then multiple group host
...
class GroupBridge {
   ...
   private:
     ...
     * the vstbridge instance can tell if the plugin crashed during
