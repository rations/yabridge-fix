 ...
 * on that.
 */
constexpr size_t vstbridge_ptr2_magic = 0xdeadbeef + 420;
...
Vst2Bridge& get_bridge_instance(const AEffect* plugin) {
    if (plugin && reinterpret_cast<size_t>(plugin->ptr2) == vstbridge_ptr2_magic)
        [[likely]] {
...
Vst2Bridge::Vst2Bridge(MainContext& main_context,
    ...
    plugin_->ptr1 = this;
    plugin_->ptr2 = reinterpret_cast<void*>(vstbridge_ptr2_magic);
    ...
    sockets_.host_plugin_control_.send(
        Vst2EventResult{.return_value = 0,
                        .payload = *plugin_,
                        .value_payload = vstbridge_git_version});
