...
void* find_plugin_library(const std::string& name) {
    ...
    const auto impl = [&name]() -> void* {
        ...
        // `dlopen()` for distro packaged versions of vstbridge
        const std::vector<fs::path> search_path = get_augmented_search_path();
        if (const auto& vstbridge_host_path =
                search_in_path(search_path, vstbridge_host_name)) {
            const fs::path candidate = vstbridge_host_path->parent_path() / name;
            if (fs::exists(candidate)) {
                return dlopen(candidate.c_str(), RTLD_LAZY | RTLD_LOCAL);
            }
        }

        if (const auto& vstbridge_host_32_path =
                search_in_path(search_path, vstbridge_host_name_32bit)) {
            const fs::path candidate =
                vstbridge_host_32_path->parent_path() / name;
            if (fs::exists(candidate)) {
                return dlopen(candidate.c_str(), RTLD_LAZY | RTLD_LOCAL);
            }
        }
    ...
    if (!handle) {
        ...
        send_notification("Could not find '" + name + "'",
                          ...
                          "instructions from vstbridge's readme",
                          this_plugin_path);
    }
...
void log_failing_dlsym(const std::string& library_name,
    ...
    logger.log("Could not find '" + std::string(function_name) + "' in '" +
               library_name + "'");
    logger.log("");
    logger.log("Try rerunning 'vstbridgectl sync'.");
    logger.log("");
    logger.log("Source: '" + this_plugin_path.string() + "'");
    logger.log("");
    ...
    send_notification("Could not find '" + std::string(function_name) +
                          "' in '" + library_name + "'",
                      "Try rerunning 'vstbridgectl sync'.", this_plugin_path);
}
