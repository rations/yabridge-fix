...
// `dlopen()` the actual `libvstbridge-{clap,vst2,vst3}.so` files and forward the
...
// but more importantly it also avoids the need to rerun `vstbridgectl sync`
// whenever vstbridge is updated. This is even more important when considering
...
// These functions are loaded from `libvstbridge-vst3.so` the first time
// `ModuleEntry()` gets called
Vst3PluginBridge* (*vstbridge_module_init)(const char* plugin_path) = nullptr;
void (*vstbridge_module_free)(Vst3PluginBridge* instance) = nullptr;
PluginFactory* (*vstbridge_module_get_plugin_factory)(
    Vst3PluginBridge* instance) = nullptr;

// This bridges the `vstbridge_version()` call from the plugin library. This
// function was added later, so through weird version mixing it may be missing
// on the vstbridge library.
const char* (*remote_vstbridge_version)() = nullptr;

/**
 * The bridge instance for this chainloader. This is initialized when
 * `ModuleEntry()` first gets called.
 */
std::unique_ptr<Vst3PluginBridge, decltype(vstbridge_module_free)> bridge(
    nullptr,
    nullptr);
/**
 ...
 * we'll need to load the corresponding `libvstbridge-*.so` file and fetch the
...
bool initialize_library() {
    ...
    library_handle = find_plugin_library(vstbridge_vst3_plugin_name);
    if (!library_handle) {
        return false;
    }
...
#define LOAD_FUNCTION(name)                                                 \
    do {                                                                    \
        ...
        if (!(name)) {                                                      \
            log_failing_dlsym(vstbridge_vst3_plugin_name, #name);            \
            return false;                                                   \
        }                                                                   \
    } while (false)

    LOAD_FUNCTION(vstbridge_module_init);
    LOAD_FUNCTION(vstbridge_module_free);
    LOAD_FUNCTION(vstbridge_module_get_plugin_factory);

    // This one can be a null pointer if the function does not yet exist in this
    // yabridge version
    remote_vstbridge_version =
        reinterpret_cast<decltype(remote_vstbridge_version)>(
            dlsym(library_handle, "vstbridge_version"));
...
extern "C" VSTBRIDGE_EXPORT bool ModuleEntry(void*) {
    ...
    if (active_instances.fetch_add(1, std::memory_order_seq_cst) == 0) {
        ...
        bridge =
            decltype(bridge)(vstbridge_module_init(this_plugin_path.c_str()),
                             vstbridge_module_free);
        if (!bridge) {
            return false;
        }
    }
...
extern "C" VSTBRIDGE_EXPORT bool ModuleExit() {
...
extern "C" VSTBRIDGE_EXPORT PluginFactory* GetPluginFactory() {
    // The host should have called `InitModule()` first
    assert(bridge);

    return vstbridge_module_get_plugin_factory(bridge.get());
}

/**
 * This returns the actual vstbridge library's version through
 * `vstbridge_version()`. Reporting the version associated with this chainloader
 * wouldn't be very useful, and that would also cause the chainloader to be
 * rebuilt on every git commit in development.
 */
extern "C" VSTBRIDGE_EXPORT const char* vstbridge_version() {
    if (!initialize_library() || !remote_vstbridge_version) {
        return nullptr;
    }

    return remote_vstbridge_version();
}
