...
// `dlopen()` the actual `libvstbridge-{vst2,vst3}.so` files and forward the
...
// These functions are loaded from `libvstbridge-clap.so` the first time
// `clap_entry.init` gets called
using ClapPluginBridge = void;

ClapPluginBridge* (*vstbridge_module_init)(const char* plugin_path) = nullptr;
void (*vstbridge_module_free)(ClapPluginBridge* instance) = nullptr;
const void* (*vstbridge_module_get_factory)(ClapPluginBridge* instance,
 ...
 */
std::unique_ptr<ClapPluginBridge, decltype(vstbridge_module_free)> bridge(
    nullptr,
    nullptr);
/**
...
bool initialize_library() {
    ...
    library_handle = find_plugin_library(vstbridge_clap_plugin_name);
    if (!library_handle) {
        return false;
    }
...
#define LOAD_FUNCTION(name)                                                 \
    do {                                                                    \
        ...
        if (!(name)) {                                                      \
            log_failing_dlsym(vstbridge_clap_plugin_name, #name);            \
            return false;                                                   \
        }                                                                   \
    } while (false)
    ...
    LOAD_FUNCTION(vstbridge_module_get_factory);
...
const void* clap_entry_get_factory(const char* factory_id) {
    ...
    return vstbridge_module_get_factory(bridge.get(), factory_id);
}
