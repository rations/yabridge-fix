...
AEffect* (*vstbridge_plugin_init)(audioMasterCallback host_callback,
...
bool initialize_library() {
    ...
    library_handle = find_plugin_library(vstbridge_vst2_plugin_name);
    if (!library_handle) {
        return false;
    }
...
#define LOAD_FUNCTION(name)                                                 \
    do {                                                                    \
        ...
        if (!(name)) {                                                      \
            log_failing_dlsym(vstbridge_vst2_plugin_name, #name);            \
            return false;                                                   \
        }                                                                   \
    } while (false)

    LOAD_FUNCTION(vstbridge_plugin_init);
...
extern "C" VSTBRIDGE_EXPORT AEffect* VSTPluginMain(
    ...
    return vstbridge_plugin_init(host_callback, this_plugin_path.c_str());
}
...
//      safe.
extern "C" VSTBRIDGE_EXPORT AEffect* deprecated_main(
    audioMasterCallback audioMaster) asm("main");
VSTBRIDGE_EXPORT AEffect* deprecated_main(audioMasterCallback audioMaster) {
