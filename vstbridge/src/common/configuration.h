 ...
 *    path to the copy of or symlink to `libvstbridge-{clap,vst2,vst3}.so` that
 *    the plugin host has tried to load.
 * 2. We start looking for a file named `vstbridge.toml` in the same directory as
 ...
 *    path between the found `vstbridge.toml` and the `.so` file. As a rule of
 *    thumb, if the `find <pattern> -type f` command executed in Bash would list
 *    the `.so` file, then the following table in `vstbridge.toml` would also
...
class Configuration {
   public:
     ...
     * file instead of using pipes to intersperse it with vstbridge's other
     ...
     * `<temporary_directory>/vstbridge-plugin-output.log`, or it can be set to
    ...
    /**
     * Use XEmbed instead of vstbridge's normal editor embedding method. Wine's
     ...
     * then vstbridge will use the 64-bit version by default. This option
