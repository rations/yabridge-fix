...
struct PluginInfo {
   public:
     ...
     * `libvstbridge-{clap,vst2,vst3}.so` file and the type of the plugin we're
     ...
     * where you use symlinks to copies of `libvstbridge-vst2.so`.
     *
     * For VST3 plugins there is a strict format as defined by Steinberg, and
     * we'll have vstbridgectl create a 'merged bundle' that also contains the
     * Windows VST3 plugin.
     *
...
/**
 * Finds the Wine plugin host (either `vstbridge-host.exe` or
 * `vstbridge-host-32.exe` depending on the plugin). For this we will search in
 * two places:
 *
 *   1. Alongside libvstbridge-{clap,vst2,vst3}.so if the file got symlinked.
 *      This is useful when developing, as you can simply symlink the
 *      `libvstbridge-{clap,vst2,vst3}.so` file in the build directory without
 *      having to install anything to /usr.
 *   2. In the regular search path, augmented with `~/.local/share/vstbridge` to
 ...
 * in the form
 * `/run/user/<uid>/vstbridge-group-<group_name>-<wine_prefix_id>-<architecture>.sock`.
 ...
 * `libvstbridge-{clap,vst2,vst3}.so`. If no configuration file could be found
 ...
 *
 * @param plugin_path The path to the .so file that's being loaded.by the VST
 ...
 * @see Configuration
 */
Configuration load_config_for(const ghc::filesystem::path& plugin_path);
