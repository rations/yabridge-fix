...
ProcessEnvironment PluginInfo::create_host_env() const {
    ...
    // of X11. This would break vstbridge's embedding and drag-and-drop handling.
...
fs::path find_plugin_library(const fs::path& this_plugin_path,
                             PluginType plugin_type,
                             bool prefer_32bit_vst3) {
    // TODO: We only consider lower case extensions, and vstbridgectl also
    ...
    switch (plugin_type) {
        case PluginType::clap: {
            ...
            if (fs::exists(plugin_path)) {
                ...
                // This is the default in vstbridgectl 4.0.
            ...
            // symlink to support links to copies of `libvstbridge-clap.so` as
            ...
            throw std::runtime_error(
                ...
                "a vstbridge '.clap' file with the same name.");
        } break;
        case PluginType::vst2: {
            ...
            // In case this files does not exist and our `.so` file is a
            // symlink, we'll also repeat this check after resolving that
            // symlink to support links to copies of `libvstbridge-vst2.so` as
            // described in issue #3
            fs::path alternative_plugin_path = fs::canonical(this_plugin_path);
            ...
            throw std::runtime_error("'" + plugin_path.string() +
                                     ...
                                     "'libvstbridge-vst2.so' to match a "
                                     "VST plugin .dll file.");
        } break;
        case PluginType::vst3: {
            ...
            // allowed to be standalone, so for vstbridge this should also always
            ...
            // Quick check in case the plugin was set up without vstbridgectl,
            // since the format is very specific and any deviations from that
            // will be incorrect.
            if (bundle_home.extension() != ".vst3") {
                throw std::runtime_error(
                    "'" + this_plugin_path.string() +
                    "' is not inside of a VST3 bundle. Use vstbridgectl to "
                    "set up vstbridge for VST3 plugins or check the readme "
            ...
            //   vstbridgectl in `~/.vst3/vstbridge`. The plugin can be either
            //   32-bit or 64-bit. If both exist, then we'll take the 64-bit
            //   version, unless the `vst3_prefer_32bit` vstbridge.toml option
            ...
            // _actual_ (with vstbridgectl `x86_64-win` should only contain a
            ...
            throw std::runtime_error(
                "'" + bundle_home.string() +
                "' does not contain a Windows VST3 module. Use vstbridgectl to "
                "set up vstbridge for VST3 plugins or check the readme "
                "for the correct format.");
        } break;
        default:
...
std::string create_logger_prefix(const fs::path& endpoint_base_dir) {
    ...
    // prefix, but strip the `vstbridge-` part since that's redundant
    std::string endpoint_name = endpoint_base_dir.filename().string();

    constexpr std::string_view socket_prefix("vstbridge-");
...
fs::path find_plugin_host(const ghc::filesystem::path& this_plugin_path,
    ...
    auto host_name = vstbridge_host_name;
    if (plugin_arch == LibArchitecture::dll_32) {
        host_name = vstbridge_host_name_32bit;
    }
...
ghc::filesystem::path generate_group_endpoint(
    ...
    socket_name << "vstbridge-group-" << group_name << "-"
                << std::to_string(
...
Configuration load_config_for(const fs::path& plugin_path) {
    // First find the closest `vstbridge.toml` file for the plugin, falling back
    // to default configuration settings if it doesn't exist
    const std::optional<fs::path> config_file =
        find_dominating_file("vstbridge.toml", plugin_path);
    if (!config_file) {
        return Configuration();
    }

    try {
        return Configuration(*config_file, plugin_path);
        ...
        send_notification(
            "Failed to parse vstbridge.toml file",
