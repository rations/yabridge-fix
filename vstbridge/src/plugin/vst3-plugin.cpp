...
// vstbridge works very differently from hosting VST2 plugin. Even with
...
// we'll use. The installation format for vstbridge will thus have the Windows
...
// loaded directly (the vstbridge <4.0 way), or they can be loaded indirectly
// from `yabridge-chainloader-*.so` (the vstbridge >=4.0 way). The advantage of
// chainloading this library from a tiny stub library is that vstbridge can be
...
// that call the `vstbridge_module_` functions exported at the bottom of
// this file.
bool InitModule() {
    assert(!bridge);
 ...
 */
extern "C" VSTBRIDGE_EXPORT Steinberg::IPluginFactory* PLUGIN_API
GetPluginFactory() {
 ...
 * using the `vstbridge_module_free()` function. If the bridge could not
 * initialize due to an error, then the error will be logged and a null pointer
 * will be returned.
 */
extern "C" VSTBRIDGE_EXPORT Vst3PluginBridge* vstbridge_module_init(
...
/**
 * Free a bridge instance returned by `vstbridge_module_init`.
 */
extern "C" VSTBRIDGE_EXPORT void vstbridge_module_free(
 ...
 * chainloaders.
 */
extern "C" VSTBRIDGE_EXPORT Steinberg::IPluginFactory*
vstbridge_module_get_plugin_factory(Vst3PluginBridge* instance) {
    assert(instance);
