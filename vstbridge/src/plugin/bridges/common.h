...
class PluginBridge {
   ...
   protected:
    ...
    void log_init_message() {
        ...
        init_msg << "Initializing vstbridge version " << vstbridge_git_version
#ifdef __i386__
                 << " (32-bit build)"
#endif
                 << std::endl;
            ...
            if (auto rttime_limit = get_rttime_limit()) {
                if (*rttime_limit != RLIM_INFINITY &&
                    *rttime_limit < rttime_min_safe_threshold) {
                    ...
                    init_msg << "   using PipeWire. vstbridge may crash when "
                                "loading plugins"
                             << std::endl;
        ...
        if (auto memlock_limit = get_memlock_limit()) {
            if (*memlock_limit != RLIM_INFINITY &&
                *memlock_limit < memlock_min_safe_threshold) {
                ...
                init_msg << std::endl;
                init_msg
                    << "   With a low memory locking limit, vstbridge may not be"
                    << std::endl;
            ...
            while (!st.stop_requested()) {
                if (!plugin_host_->running()) {
                    ...
                    send_notification(
                        ...
                        "Check vstbridge's output for more information on what "
     ...
     * version of vstbridge than this library. vstbridge may still work (and we do
    ...
    void warn_on_version_mismatch(const std::string& host_version) {
        if (host_version != vstbridge_git_version) {
            generic_logger_.log(
                "WARNING: The host application's version does not match");
            generic_logger_.log(
                "         this plugin's. Check the logs to see which vstbridge");
            generic_logger_.log(
                "         library and host are being loaded, and try "
                "rerunning");
            generic_logger_.log("         'vstbridgectl sync'.");

            send_notification(
                "Version mismatch",
                "Check the logs to see which vstbridge library and host are "
                "being loaded, and try rerunning 'vstbridgectl sync'.",
                info_.native_library_path_);
        }
    }
    ...
    /**
     * The configuration for this instance of vstbridge. Set based on the values
     * from a `vstbridge.toml`, if it exists.
     *
    ...
    /**
     * The logging facility used for this instance of vstbridge. See
