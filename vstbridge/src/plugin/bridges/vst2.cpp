                ...
                switch (event.opcode) {
                    ...
                    case audioMasterDeadBeef:
                        ...
                        logger_.log(
                            ...
                            "by");
                        logger_.log("   vstbridge. Ignoring the request.");
                        logger_.log("");
   ...
   public:
    ...
    Vst2Event::Payload read_data(const int opcode,
        ...
        switch (opcode) {
            ...
            //       when vstbridge would try to read from it. Even though no
...
intptr_t Vst2PluginBridge::dispatch(AEffect* /*plugin*/,
    ...
    switch (opcode) {
        ...
        case effEditIdle: {
            // This is the only place where we'll deviate from vstbridge's
            // 'one-to-one passthrough' philosophy. While in practice we can
            // just pass through `effEditIdle` and we have been doing so until
            // vstbridge (upstream 3.x), in reality it's much more practical to just run
